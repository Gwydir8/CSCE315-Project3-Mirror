...
class Logic_Gates{
	protected:
		...
		int output;
	public:
		//This first constructor used for the NOT Gate. Takes in one input to evaluate.
		Logic_Gates(const int& input_1) : input_1(input_1);

		//This second constructor used for And and Or Gates. Takes in two inputs to evaluate.
		Logic_Gates(const int& input_1, const int& input_2) : input_1(input_1), input_2(input_2);

		//Simple Get and Set functions. Don't know if we really need it at the moment.
...
//And Gate: Takes in two inputs. True if both are true. False if this is not the case.
...
//Or Gate: Takes in two inputs. True if either or both are true. False if this is not the case.
